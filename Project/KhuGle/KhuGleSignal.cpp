...
KgColor24 CKhuGleSignal::ReadTexture(double u, double v) {
	u = u < 0 ? 0 : (u > 1 ? 1 : u);
	v = v < 0 ? 0 : (v > 1 ? 1 : v);
	int x = (int)(m_nW * u) % m_nW;
	int y = (int)(m_nH * (1 - v)) % m_nH;
	KgColor24 color = KG_COLOR_24_RGB(m_Red[y][x], m_Green[y][x], m_Blue[y][x]);
	return color;
}
